...
```
用户输入 → Planner(规划师)  — deepseek-reasoner
            ├─ handoff → Recon(侦察兵)    — 信息收集/端口扫描/指纹识别  — deepseek-chat
            ├─ handoff → Exploit(利用手)   — Python Exploit开发/实际利用 — deepseek-chat
            └─ handoff → Report(报告员)    — 证据整理/漏洞定级/报告输出 — deepseek-reasoner
```

| Agent | 模型 | 职责 | 工具权限 |
|-------|------|------|---------|
| **Planner** | deepseek-reasoner | 任务拆解、Agent调度、全局控制 | 只读 + handoff |
| **Recon** | deepseek-chat | 资产测绘、端口扫描、JS分析、CVE匹配 | bash + 扫描工具 |
| **Exploit** | deepseek-chat | Python Exploit开发、危害提升、证据提取 | bash + 文件读写 |
| **Report** | deepseek-reasoner | 危害复核、漏洞定级、固定格式报告 | 只读 + 文件写入 |
...
### 🖥️ 实时状态栏
```
🔍 RECON  ● 🔍 ───○ 💥───○ 📋  |  deepseek-chat  |  bal:¥110.00
💥 EXPLOIT  ○ 🔍 ───● 💥───○ 📋  |  deepseek-chat  |  bal:¥110.00
📋 REPORT  ○ 🔍 ───○ 💥───● 📋  |  deepseek-reasoner  |  bal:¥110.00
```
