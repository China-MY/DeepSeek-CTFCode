...
用法：
  ...
  ctfcode config memory-v5 [off|observe|compact|on|status]  配置 Memory v5
  ctfcode config reasoning-language [auto|zh|en]        配置可见思考语言
  ctfcode mcp <add|remove|list|import>                 管理 reasonix.toml 里的 MCP 服务器
  ctfcode init                                         查看如何生成项目记忆（AGENTS.md）
  ctfcode doctor [--json]                              输出脱敏的本地诊断信息
  ctfcode doctor session <branch-id> [--zip] [--out PATH]  导出会话冲突诊断 zip
  ctfcode bot start|doctor|weixin-login                多渠道 IM bot 网关
  ctfcode upgrade [--check] [--force]                   自更新到最新版本（也可用：ctfcode update）
  ctfcode version
  ctfcode help
