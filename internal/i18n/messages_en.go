...
Usage:
  ...
  ctfcode config memory-v5 [off|observe|compact|on|status]  configure Memory v5
  ctfcode config reasoning-language [auto|zh|en]        configure visible reasoning language
  ctfcode mcp <add|remove|list|import>                 manage MCP servers in reasonix.toml
  ctfcode init                                         show how to generate project memory (AGENTS.md)
  ctfcode doctor [--json]                              print redacted local diagnostics
  ctfcode doctor session <branch-id> [--zip] [--out PATH]  export a session conflict diagnostic zip
  ctfcode bot start|doctor|weixin-login                multi-channel IM bot gateway
  ctfcode upgrade [--check] [--force]                   self-update to the latest release (also: reasonix update)
  ctfcode version
  ctfcode help
