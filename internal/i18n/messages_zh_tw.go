...
var ChineseTraditional = Messages{
	...
	UsageBody: `ctfcode — 由設定和插件驅動的 coding agent（多模型）

用法：
  ctfcode [--model NAME] [-c|--continue] [--resume] [--yolo] [--dir PATH]   互動式會話（多輪；-c 恢復最近一次，--resume 選擇一個）
  ctfcode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   執行單次任務後退出
  ctfcode review [--base BRANCH] [--commit SHA] [--model NAME]  AI 程式碼審查（基於本機 diff）
  ctfcode serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  透過 HTTP+SSE 提供服務（支援可選認證）
  ctfcode acp [--model NAME]                           透過 stdio 提供 Agent Client Protocol（也可用：ctfcode --acp）
  ctfcode setup [path]                                 互動式設定精靈；生成 reasonix.toml（及 .env）
  ctfcode config auto-plan [off|on]                    設定自動計畫模式
  ctfcode config memory-v5 [off|observe|compact|on|status]  設定 Memory v5
  ctfcode config reasoning-language [auto|zh|en]        設定可見思考語言
  ctfcode mcp <add|remove|list|import>                 管理 reasonix.toml 裡的 MCP 伺服器
  ctfcode init                                         查看如何產生專案記憶（AGENTS.md）
  ctfcode doctor [--json]                              輸出脫敏的本機診斷資訊
  ctfcode doctor session <branch-id> [--zip] [--out PATH]  匯出會話衝突診斷 zip
  ctfcode bot start|doctor|weixin-login                多管道 IM bot 閘道
  ctfcode upgrade [--check] [--force]                   自更新至最新版本（也可用：ctfcode update）
  ctfcode version
  ctfcode help
...
範例：
  ctfcode
  ctfcode --continue
  ctfcode run "把 main.go 裡的 TODO 實現掉"
  ctfcode run --model mimo-pro "給這個函式補單元測試"
