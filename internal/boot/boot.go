...
import (
	...
	"ctfcode/internal/agent"
	"ctfcode/internal/command"
	"ctfcode/internal/config"
	"ctfcode/internal/control"
	"ctfcode/internal/environment"
	"ctfcode/internal/event"
	"ctfcode/internal/guardian"
	"ctfcode/internal/history"
	"ctfcode/internal/hook"
	"ctfcode/internal/installsource"
	"ctfcode/internal/instruction"
	"ctfcode/internal/jobs"
	"ctfcode/internal/knowledge"

	"ctfcode/internal/lsp"
	"ctfcode/internal/memory"
	"ctfcode/internal/memorycompiler"
	"ctfcode/internal/migration"
	"ctfcode/internal/netclient"
	"ctfcode/internal/outputstyle"
	"ctfcode/internal/permission"
	"ctfcode/internal/planmode"
	"ctfcode/internal/plugin"
	"ctfcode/internal/provider"
	"ctfcode/internal/sandbox"
	"ctfcode/internal/skill"
	"ctfcode/internal/tool"
	"ctfcode/internal/tool/builtin"
	"ctfcode/internal/tool/sessiontool"
)
...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	// the executor in an Orchestrator when [[agents]] is configured.
	handoffBus := tool.NewHandoffBus()
	builtin.RegisterHandoffTools(handoffBus)
	...
	// Multi-agent orchestration: when [[agents]] is configured, wrap the
	// executor in an Orchestrator that manages multiple specialized agents
	// with handoff support. The handoff bus is shared between the tools and
	// the orchestrator.
	if len(cfg.Agents) > 0 && !tokenEconomy {
		...
		for _, e := range cfg.Agents {
			...
			agentRegistry.Add(agent.AgentConfig{
				ID:              agentID,
				Name:            e.Name,
				Role:            e.Role,
				SystemPrompt:    e.SystemPrompt,
				SystemPromptFile: e.SystemPromptFile,

				Provider:        e.Provider,
				Model:           e.Model,
				AllowedTools:    e.AllowedTools,
				AllowedHandoffs: e.AllowedHandoffs,
				MountedSkills:    e.MountedSkills,
				MountedKnowledge: e.MountedKnowledge,
		...
		if agentRegistry.IsMultiAgent() {
			// Capture config values before the closure to avoid shadowing.
			agentTemperature := cfg.Agent.Temperature
			agentSoftCompactRatio := cfg.Agent.SoftCompactRatio
			agentToolResultSnipRatio := cfg.Agent.ToolResultSnipRatio
			agentCompactRatio := cfg.Agent.CompactRatio
			agentCompactForceRatio := cfg.Agent.CompactForceRatio
			agentRecentKeep := cfg.Agent.RecentKeep
			agentReasoningLanguage := cfg.ReasoningLanguage()
			agentArchiveDir := config.ArchiveDir()
			agentOptsFunc := func(_ agent.AgentConfig) agent.Options {
				return agent.Options{
					...
					Jobs:                 jm,
					ContextWindow:        entry.ContextWindow,
					SoftCompactRatio:     agentSoftCompactRatio,
					ToolResultSnipRatio:  agentToolResultSnipRatio,
					CompactRatio:         agentCompactRatio,
					CompactForceRatio:    agentCompactForceRatio,
					RecentKeep:           agentRecentKeep,
					ArchiveDir:           agentArchiveDir,
					KeepPolicy:           keepPolicy,
					ReasoningLanguage:    agentReasoningLanguage,
					SubagentDepth:        0,
					MaxSubagentDepth:     maxSubagentDepth,
				}
			}
			...
			// Initialize knowledge base for mounted Knowledge entries.
			knowledgeStore := knowledge.Load(knowledge.LoadOptions{
				ProjectRoot: root,
			})

			// Initialize experience store for self-learning.
			expStore := agent.NewStore(100)

			// Wrap the executor in the orchestrator.
			orchestrator := agent.NewOrchestrator(
				agentRegistry, execProv, reg, executor,
				agentOptsFunc, sink,
				agent.WithHandoffBus(handoffBus),
				agent.WithKnowledgeBase(knowledgeStore),
				agent.WithExperienceStore(expStore),

			)
			runner = orchestrator
			label = "multi-agent (" + strings.Join(agentRegistry.AgentNames(), ", ") + ")"
		}
	}
