...
func loadCredentialStoreForRoot(root string) {
	...
	if p := UserCredentialsPath(); p != "" {
		loadDotEnvFileAs(p, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "ctfcode credentials (.env)"})
	}
}
...
func pinCredentialAssignments(assignments map[string]string) {
	for key, value := range assignments {
		...
		recordCredentialSource(key, value, CredentialSource{Kind: CredentialSourceCredentials, Path: UserCredentialsPath(), Label: "ctfcode credentials (.env)"})
	}
}
...
func credentialSourceLabel(source CredentialSource) string {
	...
	case CredentialSourceCredentials:
		return "ctfcode credentials"
	case CredentialSourceHomeEnv:
		return "home .env"
	case CredentialSourceLegacy:
		return "legacy ctfcode credentials"
...
func storedCredentialValue(key string) (string, CredentialSource, bool) {
	if p := UserCredentialsPath(); p != "" {
		if value, ok := envFileValue(p, key); ok && value != "" {
			return value, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "ctfcode credentials (.env)"}, true
		}
	}
	return "", CredentialSource{}, false
}
