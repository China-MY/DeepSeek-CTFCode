...
func (r *MigrationResult) Notice() string {
	...
	if r.KeyToEnv {
		b.WriteString("; API key saved to ctfcode's credentials store")
	}
...
func MigrateLegacyIfNeededForRoot(root string) (*MigrationResult, error) {
	...
	src := filepath.Join(home, ".ctfcode", "config.json")
	...
	if qqSecret := strings.TrimSpace(legacy.QQ.AppSecret); qqSecret != "" {
		...
		res.Warnings = append(res.Warnings, "your previous QQ Bot App Secret was saved to ctfcode's credentials store")
	}
...
func legacyTOMLPaths(dest, home string) []string {
	...
	if home != "" {
		add(filepath.Join(home, ".ctfcode", "config.toml"))
	}
	return paths
}
