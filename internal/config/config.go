// Package config loads ctfcode's runtime configuration from TOML. Resolution order:
...
// Config is ctfcode's runtime configuration.
type Config struct {
...
//
// When [[agents]] is configured (see AgentEntry), multi-agent mode is activated
// and the orchestrator manages multiple specialized agents with handoff support.
...
// one agent entry is configured, the session runs in multi-agent mode with an
// Orchestrator managing handoff between agents. Each agent may specify its own
// model binding, tool ACL, and allowed handoff targets.
type AgentEntry struct {
	// ID is a short unique identifier (e.g. "planner", "operator").
	// Used as the target in handoff_to_agent calls.
	ID string `toml:"id"`

	// Name is a human-readable display label (e.g. "规划师", "执行者").
	Name string `toml:"name"`

	// Role is a one-line description of the agents function.
	Role string `toml:"role"`

	// SystemPrompt is the core instruction prompt for this agent. When empty,
	// the sessions shared system prompt is used.
	SystemPrompt string `toml:"system_prompt"`


	// SystemPromptFile is an alternative to SystemPrompt: path to a file
	// containing the system prompt. Relative paths are resolved from the
	// project root. When both are set, SystemPrompt takes precedence.
	SystemPromptFile string `toml:"system_prompt_file"`

	// Provider references the name of a configured [[providers]] entry.
	// When empty, the sessions default provider is used.
	Provider string `toml:"provider"`

	// Model overrides the default model from the resolved provider.
	// Format: "model-name" or "" for the providers default.
	Model string `toml:"model"`

	// AllowedTools is the set of tool names this agent may call.
	// A single entry "*" means all tools. Empty means no tools (handoff only).
	// When not set (nil), all tools are allowed (backward compat).
	AllowedTools []string `toml:"allowed_tools"`

	// AllowedHandoffs lists agent IDs this agent may hand off to.
	// Empty means handoffs are forbidden. Nil means any (no restriction).
	AllowedHandoffs []string `toml:"allowed_handoffs"`

	// ThinkingLevel controls the models reasoning budget.
	// Values: "low", "medium", "high". Empty uses provider default.
	ThinkingLevel string `toml:"thinking_level"`

	// MountedSkills lists skill names this agent has access to.
	// When non-empty, only these skills appear in the agents prompt index.
	MountedSkills []string `toml:"mounted_skills"`

	// MountedKnowledge lists knowledge entry names to inject into this
	// agents context. Knowledge entries are Markdown files in
	// .reasonix/knowledge/ directories.
	MountedKnowledge []string `toml:"mounted_knowledge"`

	// EnableLearning enables automatic experience recording.
	// When true, the agent persists key decisions and outcomes.
