...
func reasonixHomeDir() string {
	...
	if runtimeGOOS == "windows" {
		if dir := osUserConfigDir(); dir != "" {
			return filepath.Join(dir, "ctfcode")
		}
		if home, err := osUserHomeDir(); err == nil && home != "" {
			return filepath.Join(home, "AppData", "Roaming", "ctfcode")
		}
		return ""
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		return filepath.Join(home, ".ctfcode")
	}
	if dir := osUserConfigDir(); dir != "" {
		return filepath.Join(dir, "ctfcode")
	}
...
func legacyXDGConfigPaths() []string {
	...
	if dir := cleanEnvDir("XDG_CONFIG_HOME"); dir != "" {
		add(filepath.Join(dir, "ctfcode", "config.toml"))
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "ctfcode", "config.toml"))
	}
	return paths
}
...
var ConventionDirs = []string{".ctfcode", ".agents", ".agent", ".claude"}
