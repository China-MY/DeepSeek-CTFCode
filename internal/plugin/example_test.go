...
// buildExamplePlugin compiles cmd/ctfcode-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "ctfcode-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "ctfcode/cmd/ctfcode-plugin-example").CombinedOutput()
	if err != nil {
