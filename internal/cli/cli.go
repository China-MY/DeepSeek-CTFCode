...
import (
	...
	"ctfcode/internal/provider/openai"
	"ctfcode/internal/sandbox"
	"ctfcode/internal/serve"
	"time"
...
func Run(args []string, version string) int {
	...
	case "version", "--version", "-v":
		fmt.Println("DeepSeek-CTFCode", version, "- by Mr.明裕")
		return 0
	case "help", "--help", "-h":
		usage()
		return 0
...
func runServe(args []string) int {
	...
	*model = modelForResumePath(*model, *resume, cfg)
	...
	fmt.Printf("ctfcode serve — %s on http://%s\n", ctrl.Label(), *addr)
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("ctfcode")),
		"",
