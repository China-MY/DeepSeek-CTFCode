...
// mcp.go holds the MCP server-management surface shared by the `ctfcode mcp`
...
// mcpCommand implements `ctfcode mcp <add|remove|list>`. It edits config only
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: ctfcode mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to ctfcode.toml).

Usage:
  ctfcode mcp list
  ctfcode mcp add <name> <command> [args...]        stdio server
  ctfcode mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  ctfcode mcp add <name> --sse  <url>               remote (legacy SSE)
  ctfcode mcp import                                import MCP servers from cc-switch
  ctfcode mcp remove <name>
...
Examples:
  ctfcode mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  ctfcode mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
